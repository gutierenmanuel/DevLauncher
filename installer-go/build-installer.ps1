...
function New-WindowsIconResources {
    ...
    Write-Color "  Icono aplicado a installer.exe" Gray
}
...
foreach ($stale in @("$VersionNumber-devlauncher-uninst.exe", "$VersionNumber-devlauncher-uninst-linux")) {
...
function Clear-Assets {
    ...
    foreach ($f in @("VERSION.txt","launcher.exe","launcher-linux","launcher-mac","uninstaller.exe","uninstaller-linux","uninstaller.sh","uninstaller.ps1")) {
        $p = Join-Path $AssetsDir $f
        if (Test-Path $p) { Remove-Item $p -Force }
    }
}

function Prepare-AssetsForTarget([string]$TargetLauncherName, [string]$TargetLauncherDest) {
    Write-Step "Preparando assets para $TargetLauncherDest..."
    Clear-Assets
...
Write-Color "  Uninstaller ligero generado en instalación (script local)" Gray
