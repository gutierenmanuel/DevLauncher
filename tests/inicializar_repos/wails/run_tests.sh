...
# Test runner para scripts de inicialización de proyectos Wails
...
SCRIPTS_DIR="$REPO_ROOT/scripts/linux/inicializar_repos/wails"
...
echo -e "${PURPLE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║   Tests: Inicialización de Repos Wails 🖥️                  ║${NC}"
echo -e "${PURPLE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
run_test() {
    ...
    if (cd "$test_dir" && yes | eval "$env_vars" bash "$script") > "$log_file" 2>&1; then
        local end_time
        end_time=$(date +%s)
...
# ==========================================
# TEST: init_wails_project.sh
# ==========================================
run_test "wails_project" \
    "$SCRIPTS_DIR/init_wails_project.sh" \
    ""
