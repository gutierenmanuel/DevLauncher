...
# Test runner para scripts de inicialización de proyectos Python
...
SCRIPTS_DIR="$REPO_ROOT/scripts/linux/inicializar_repos/python"
...
echo -e "${PURPLE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║   Tests: Inicialización de Repos Python 🐍                 ║${NC}"
echo -e "${PURPLE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
run_test() {
    ...
    if command -v tree &>/dev/null; then
        tree "$test_dir" -L 3 --dirsfirst -I '_test.log|.venv|__pycache__' 2>/dev/null | head -30 | sed 's/^/    /'
    else
        find "$test_dir" -maxdepth 3 -not -name '_test.log' -not -path '*/.venv/*' -not -path '*__pycache__*' | sort | head -30 | sed 's/^/    /'
    fi
    echo ""
}

# ==========================================
# TEST: init_python_project.sh
# ==========================================
run_test "python_project" \
    "$SCRIPTS_DIR/init_python_project.sh" \
    ""
