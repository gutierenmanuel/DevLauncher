...
select_operation() {
    echo ""
    echo "Conversor de video:"
    echo "1) Convertir formato de video"
    echo "2) Extraer audio del video"
...
select_target_ext() {
    ...
    if [[ "$operation" == "convert" ]]; then
        echo "Formato de salida de video (ej: mp4, webm, mkv, mov, avi):"
    else
        echo "Formato de salida de audio (ej: mp3, wav, flac, aac, ogg):"
    fi
...
main() {
    ...
    local target_ext
    target_ext="$(select_target_ext "$operation")" || pause_and_exit 1
    ...
    if [[ "$mode" == "single" ]]; then
        local file_path
        read -r -p "Ruta del archivo de video: " file_path
    ...
    local filter_ext
    read -r -p "Extensión origen a procesar (ej: mp4, webm, mkv): " filter_ext
