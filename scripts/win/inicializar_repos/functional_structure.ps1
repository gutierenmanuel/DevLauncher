# Script: Crea una estructura funcional completa de carpetas para un repositorio.
...
$BasePath = Resolve-Path -Path $Destino -ErrorAction Stop
...
foreach ($Dir in $Directories) {
    $Target = Join-Path -Path $BasePath -ChildPath ($Dir -replace '/', [IO.Path]::DirectorySeparatorChar)
    New-Item -Path $Target -ItemType Directory -Force | Out-Null
}

Write-Host "✅ Estructura creada en: $BasePath"
