# Nuevas rules en DevLauncher

Usa estas reglas cuando crees un archivo nuevo en `robots/rules/`.

## 1) Cuándo crear una rule nueva

Crea una rule nueva solo si:

- hay una tarea repetida que no está bien cubierta por una rule existente,
...
Si el objetivo ya está cubierto, amplía la rule existente en lugar de duplicar contenido.
...
## 3) Estructura mínima de la rule

Toda rule nueva debería incluir:
...
- Evitar contradicciones con `copilot-instructions.md` y rules existentes.
- Si una regla aplica a varias áreas, enlazarla en el índice en vez de duplicarla.

## 5) Integración obligatoria con el índice

Cada nuevo archivo en `robots/rules/` debe añadirse en `robots/rules/index.md` con:
...
- Si cambian convenciones del repo, actualiza primero la rule afectada.
- Evita crear rules casi idénticas; fusiona cuando sea posible.
- En cambios grandes, añade una mini sección de “errores comunes” si aporta valor.

## 7) Checklist rápido

- [ ] El problema no está cubierto por otra rule existente.
- [ ] El nombre del archivo es claro y consistente.
- [ ] La rule tiene objetivo, uso, reglas y checklist.
- [ ] No contradice instrucciones del repositorio.
- [ ] Se agregó/actualizó entrada en `robots/rules/index.md`.
